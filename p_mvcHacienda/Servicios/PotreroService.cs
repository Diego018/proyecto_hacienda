...
namespace p_mvcHacienda.Servicios 
{
    public class PotreroService : IPotreroService 
    {
        ...
        public string CrearPotrero(string identificacion, l_tipos_potreros tipo) 
        {
            ...
            {
                throw new Exception($"Error al crear el potrero: {ex.Message}", ex);
            }
        }
        ...
        public string AgregarRes(string potreroId, Res res) 
        {
            try 
            {
                ...
                // 1. Validaciones de Dominio (AS-IS)
                ...
                // 2. Suscripción a Observadores Nativos de Dominio
                potrero.PotreroLleno += ReglaPotrero.OnPotreroLleno;
                potrero.PotreroMitad += ReglaPotrero.OnPotreroMitad;

                // 3. Ejecución y Notificación
                string resultado = potrero.anadir_res(res);

                // Disparo de eventos en caso de cumplir condiciones de umbral
                ushort cantidadNueva = (ushort)potrero.obtener_reses().Count;
                if (cantidadNueva >= ReglaPotrero.max_reses_potrero)
                {
                    potrero.OnPotreroLleno(new PotreroEventArgs(potrero));
                }
                else if (cantidadNueva >= (ReglaPotrero.max_reses_potrero / 2))
                {
                    potrero.OnPotreroMitad(new PotreroEventArgs(potrero));
                }
            ...
            {
                throw new Exception($"Error al agregar la res: {ex.Message}", ex);
            }
        }
    }
}